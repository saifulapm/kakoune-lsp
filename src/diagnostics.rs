...
fn severity_to_face(severity: Option<DiagnosticSeverity>, prefix: &str) -> &'static str {
    match severity {
        Some(DiagnosticSeverity::ERROR) => match prefix {
            "Inlay" => "InlayDiagnosticError",
            _ => "DiagnosticError",
        },
        Some(DiagnosticSeverity::HINT) => match prefix {
            "Inlay" => "InlayDiagnosticHint",
            _ => "DiagnosticHint",
        },
        Some(DiagnosticSeverity::INFORMATION) => match prefix {
            "Inlay" => "InlayDiagnosticInfo",
            _ => "DiagnosticInfo",
        },
        Some(DiagnosticSeverity::WARNING) | None => match prefix {
            "Inlay" => "InlayDiagnosticWarning",
            _ => "DiagnosticWarning",
        },
        Some(_) => match prefix {
            "Inlay" => "InlayDiagnosticWarning",
...
pub fn publish_diagnostics(server_id: ServerId, params: Params, ctx: &mut Context) {
    ...
    for (server_id, diagnostic) in diagnostics {
        let face = severity_to_face(diagnostic.severity, "Inlay");
